...
set +e
trap 'cleanup_e2e; for ns in "e2e-team-alpha-${E2E_RUN_ID}" "e2e-team-beta-${E2E_RUN_ID}" "e2e-inject-${E2E_RUN_ID}" "e2e-ns-a-${E2E_RUN_ID}" "e2e-ns-b-${E2E_RUN_ID}" "e2e-backup-${E2E_RUN_ID}"; do kubectl delete namespace "$ns" --ignore-not-found --wait=false 2>/dev/null || true; done' EXIT
