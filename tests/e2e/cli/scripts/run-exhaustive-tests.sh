...
if command -v age-keygen > /dev/null 2>&1 && command -v sops > /dev/null 2>&1; then
    # Use the key generated by cfgd secret init (SEC02) so encrypt/decrypt
    # use the same key cfgd's sops backend will use
    CFGD_AGE_KEY="$CFG/age-key.txt"
    if [ ! -f "$CFGD_AGE_KEY" ]; then
        # Fallback: generate key in cfgd config dir
