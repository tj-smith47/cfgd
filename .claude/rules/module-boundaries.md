...
| `cfgd-core/src/util/process.rs::tool_cmd` / `systemctl_cmd` | the seam-honouring `Command` factories. `tool_cmd(env_var, default)` is the generic one; `systemctl_cmd()` is the named `systemctl` case, because call sites in both binary-bearing crates spawn it and a test can only redirect them all through one spelling of `CFGD_SYSTEMCTL_BIN`. Every `systemctl` invocation in the workspace goes through it — `system/systemd_unit.rs`, `system/node/{kubelet,containerd}.rs`, `generate/scan`, `daemon/service/systemd.rs`, `util/env_session.rs` — and each pairs it with `command_output_with_timeout`. `reg_cmd()` is the same named case for `reg`, shared by `util/env_session.rs` and `system/windows_registry.rs` through `CFGD_REG_BIN`. Re-exported as `cfgd_core::tool_cmd` / `cfgd_core::systemctl_cmd` / `cfgd_core::reg_cmd` |
| `cfgd-core/src/providers/mod.rs` | **signature only** — `SystemContext::run_silent` names `&mut std::process::Command` and forwards to `output/`. A `SystemConfigurator` builds the `Command` in `system/` and hands it here; nothing is constructed or spawned in this file |
| `cfgd-core/src/util/env_session.rs` | controlled layer for **user-session live environment refresh** — `launchctl setenv` (macOS), `systemctl --user set-environment` (Linux), `setx` (Windows), plus their read-back getters. The single home for these shell-outs, consumed by `reconciler::apply_env_action` (user-scope `spec.env`) and `system/environment` (system configurator). Re-exported as `cfgd_core::launchctl_setenv` / `cfgd_core::windows_setx` / `cfgd_core::refresh_session_env` |
