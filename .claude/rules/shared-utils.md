...
- `cli::status::drift_checked_note(last_scan_at, now)` — the ONE human rendering of the recorded drift-scan freshness (`checked 3m ago` / `drift never checked`), read by the Component Health heading annotation AND the Drift verdict qualifier; the age degrades through `humanize_age_cell` like every other age slot.
