...
- **A module inventory row with no check behind it renders as a declaration, never a verdict** — `render_module_inventories`' Shell rows fall to `command_list` (bare name, no glyph) unless `drift_checked_live` stands behind them, and Scripts rows are always `command_list`, no drift engine ever watching a hook body. `no_declared_inventory_row_wears_a_verdict_glyph` extends `no_recorded_verdict_claims_a_check_that_never_ran` to every inventory row.
