...
- `cli::live_drift::is_presence_package_row(r)` — which half of the package walk a `"package"` finding came from, for a surface filing rows by shape; the presence pass's two `expected` words are its whole vocabulary.
- `reconciler::split_package_drift_resource_id(id)` — the inverse of `package_drift_resource_id`, kept beside its producer; for RENDERING a row's parts, never for re-deriving a stored id.
