...
- **A fleet drift row's `field` is `<configurator>.<key>`** — `compliance::system_drifts` yields each drift paired with the configurator that reported it and `server_client::report_drift` composes the wire value through `reconciler::system_resource_key`; no `system:` prefix (that wrapper is resource-id grammar), and every gateway reader copies the value whole. `every_fleet_drift_field_comes_from_the_one_composer` walks the mints and the readers.
