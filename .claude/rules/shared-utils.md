...
- `system_resource_key(configurator, key)` in the same file, re-exported as `cfgd_core::reconciler::system_resource_key` — the ONE composition of a system setting's `<configurator>.<key>` identity, and the only way to build it. Three surfaces mint this string and match it against each other, so a byte of divergence between any two records drift that is never resolved: `format_action_description` (the `managed_resources` id and the journal `resource_id`), `cli::live_drift` (the `drift_events` id `resolve_drift` must find) and `compliance::collect_system_checks` (the check key `compliance diff` pairs two snapshots on). It carries a `debug_assert` that the `SystemDrift::key` does not open with its own configurator's name — six configurators once self-prefixed, doubling the name into two PERSISTED columns, and undoing that cost a state migration. The same guard is exposed as `debug_assert_system_key_undoubled(configurator, key)` for the planner (`reconciler/plan.rs`), where a configurator's `diff` output first meets its name and so every configurator is checked on every planning run rather than only the ones a test pins. Debug-only on purpose: a doubled key still composes a usable id, so a release build must not panic mid-apply over a naming defect
- `compliance::snapshot_content_hash(snapshot)` in `crates/cfgd-core/src/compliance/mod.rs` — the ONE serialization-and-hash derivation for a compliance snapshot, returning `(json, sha256_hex(json))`. `StateStore::store_compliance_snapshot` derives from it internally rather than accepting a hash, so `compliance_snapshots.content_hash` is always the digest of the `snapshot_json` beside it. Both writers (the daemon's reconcile tick and `cfgd compliance`) take their change-detection hash from here; they previously hashed different serializations of the same snapshot, so a tick following a CLI run always read "changed"
