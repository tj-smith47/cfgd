...
- `action_display_subject(action)` (`reconciler/format.rs`) — the ONE display derivation of an action's subject; a preview, an alignment column and an executed line must be one string, and a subject names EVERY operand at every width (a long list wraps, never cuts). Never persist a `DisplaySubject`. `lanes::node_subject` / `lane_occupant` / `blocker_subject` narrow it for wait lines: a blocker is named by the row the reader can see, never by a manager token or an operand list. `every_manager_action_subject_names_every_operand_it_holds` binds the variants.
