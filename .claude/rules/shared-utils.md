...
- `SIGNATURE_VERIFIED` / `SIGNATURE_UNVERIFIED` / `SIGNATURE_UNSIGNED` / `SIGNATURE_UNKNOWN` (`cfgd-crd/src/lib.rs`) — the ONE four-word vocabulary a person reads for a module's signature, written ONLY by the controller that ran the check. The `SIGNATURE` printer column binds to `status.signature`; `kubectl cfgd status` reads that same field (`MODULE_SIGNATURE_POINTER`) and spells its absence `unknown` — a Module no reconcile has written is a check that has not run, whatever its spec declares; the one exception is a raw `status.verified: true` beside no word (an operator predating the word), which is the single fact the bool asserts alone. Never derive a NEGATIVE word from `status.verified` and the spec: the bool pair cannot express `unknown`, and deriving `unverified` from a declared key reported cosign rejecting an artifact nothing had looked at. `status.verified` stays on the wire as the raw bool. `a_status_less_module_reads_unknown_on_the_plugin_and_the_crd_column_alike` pins the read.
