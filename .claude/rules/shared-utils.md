...
- `reconciler::recorded_env_method(resource_id)` (`reconciler/env_engine.rs`) — the `write`/`inject` verb a recorded env row was produced by, reconstructed from the target's basename because the recorded id drops it; welded to `env_targets` by `every_env_target_classifies_under_the_verb_that_produced_it`.
