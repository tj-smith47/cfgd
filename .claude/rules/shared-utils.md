...
- `cli::status::drift_checked_note(last_scan_at, now)` + `freshest_check_stamp(last_scan_at, row_stamps)` — the ONE human rendering of the recorded drift-scan freshness (`checked 3m ago` / `drift never checked`) and the ONE fold picking the instant it dates: the freshest of the machine-wide stamp and the recorded rows' own timestamps, feeding the Component Health annotation, the Drift verdict qualifier and both surfaces' scan-hint staleness gate; the age degrades through `humanize_age_cell` like every other age slot.
