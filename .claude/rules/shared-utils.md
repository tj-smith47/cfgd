...
- `source_success_next_step(SourceMutation)` (same file) — the SUCCESS-side twin: the hint every mutating `source` verb (`add`, `update`, `remove`, `replace`, `override`, `priority`) closes on. A `requireSignedCommits` edit points at `cfgd sync`; every composition edit at `MSG_RUN_APPLY`. A verb nested inside another's report (`replace` running `add`/`remove`) passes `closing: false` and leaves the hint to the outer verdict. `every_mutating_source_verb_closes_on_a_next_step` walks the family; `every_source_verdict_counts_iff_its_verb_takes_many_subjects` pins the sibling rule that the verdict carries a count exactly when the verb's clap arity allows several subjects.
