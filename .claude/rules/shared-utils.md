...
- `DecisionExclusions::withholds_recorded_row(rtype, rid)` (`reconciler/pending.rs`) — whether a recorded drift row names a resource the pending-decision prune withheld from the plan, over every id grammar both producers mint. Every complement-resolve reading a pruned plan consults it, or the withheld rows heal blind; `a_tick_keeps_the_rows_of_a_resource_awaiting_a_source_decision` pins the tick.
...
- `cli::status::recorded_module_tallies(items, declared)` → `ModuleTally` — what the Managed Resources table says this host manages per module, and the ONE source of the Modules headline's counts. `every_module_owned_kind_the_table_lists_has_a_slot_in_the_headline` walks `display_type`'s arms.
...
- `cli::status::managed_resource_rows(items, modules, profile)` + `owner_render_order(rows)` (`crates/cfgd/src/cli/status.rs`) — the Managed Resources rows and the ONE place its Owner column is decided and ordered: the vocabulary is `reconciler::owner_of`'s and the order is `Owner::order`, so the dashboard names and orders a resource exactly as the apply tree does. `profile` is `Option<&str>` (a `--module` run resolves none; the cell reads `-` and sorts last). Display only; `-o json` carries the raw rows.
...
- **A row's VERDICT leads its detail; its counts are the parenthetical** — `✓ module:nvim — Synced (24 packages, 6 files, 7 scripts)`: the health word a reader scans for never lands behind the inventory. `no_status_detail_trails_a_verdict_word_behind_its_counts` walks the population.
