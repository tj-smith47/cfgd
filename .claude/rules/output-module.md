...
- `status_owner_with(role, OwnerLabel, |f| …)` / `status_transition_with(subject, old, new, |f| …)` (`Doc` / `SectionBuilder`) — the two status rows whose SUBJECT the renderer PAINTS from typed parts (`PaintedSubject`) instead of coating with the row's single role: an owner token's three slots, and a state change's `(<old> → <new>)` with each status word in its own vocabulary's role. The plain form still travels as the row's `subject`. A token inside a sentence keeps the plain slot. `every_status_row_naming_an_owner_takes_the_owner_subject_slot` pins the first.
...
- `SectionBuilder::script_steps(steps, form)` — the buffered slot a module's declared scripts render through: the renderer owns each step's muted marker line, its bash highlighting and the blank line between steps, and the hook heads its steps as a plain nested subsection carrying no step count (`postApply`, never `postApply (7)`). Reach it through `cfgd_core::modules::scripts_section`; `SectionGuard::script_steps` is the same slot for a Printer-driven surface, reached through `modules::post_apply_scripts_section`; its `nested_script_steps` twin hangs one step under a status row instead of a hook heading, reached through `modules::post_apply_change_body`.
- `diff(old, new)` / `syntax_highlight(code, lang)` — nest at whatever depth is ambient, flushing any pending section header first. A highlighted block is painted in the printer's own preset through `Theme::syntect_theme` (the mapping is the table on `Theme`; the bundled `.tmTheme` assets and their licences are in `output/tmthemes/THIRD_PARTY.md`), and each highlighted line closes on a reset; a line syntect refuses carries no escape at all, and `minimal` and an asset that will not parse render the whole block plain. Never name a syntect theme at a call site. `every_preset_resolves_the_syntect_theme_it_names` walks the presets.
...
`--resolved` is the ONE spelling of the flag, per verb, never global: it changes WHAT is rendered, not how. A `show` verb's `-o yaml` is the serializer's YAML of the same payload, syntax-highlighted when colour is on, never a hand-rendered document and never the file's comments (a literal-file channel is a separate flag). A `show` verb never opens a state store, a package context, a platform probe or a manager registry outside its `--resolved` branch. **Every declared env VALUE masks by default** through `mask_value`, on every surface rendering one (`module show`, `profile show`, `source show`, `status <module>`). The run-wide decision is settled once by `cli::resolve_mask_env_values` (`--mask-env-values` > `CFGD_MASK_ENV_VALUES` > `spec.output.maskEnvValues` > mask everything) and carried on the `Printer`; a verb's `--show-values` is the per-verb spelling of `none` and conflicts with the global flag. A rendered URL never carries its userinfo. A standing drift row (one the run could not re-check) renders under its own `Standing` heading at `Role::Warn` on every surface that prices it, never as a member of the live rows.
...
| A **decisions instruction closes the surface** | the answer and the include-declined hints are closing hints: the section names the withheld items, the surface prints its verdict, and the hint renders at the run's own depth after a blank line, through `ApplyRun::withheld_hints` for a run and the document's own foot for `decide` / `status` | `every_decisions_hint_closes_the_surface_not_its_section` |
...
| A verb's **closing next step renders at the RUN's own depth** | `success_next_step(Mutation)` is the last thing a mutating verb says, addressed to the whole run, so it goes on the printer or on the `Doc` the verb emits after its section closes — never through a `SectionGuard`, which indents the run's last word to the section's depth and its `$` block one further. A hint that qualifies the ROW above it, and after which the run goes on, is a different thing and stays inside its section | `every_mutating_verbs_next_step_renders_at_the_runs_own_depth` (`crates/cfgd/src/cli/tests.rs`) |
...
| A **heading owns every block under it until a row or a paragraph is printed** | a heading's kv facts indent beneath it; a ROW ends the scope, so a verb's closing facts block stays at the report's own depth, and so does PROSE — a paragraph is the heading's last word, and `explain`'s `Location` / `Docs` pointers after it are the surface's own block at column 0. A surface ordering heading, verdict, facts loses the binding for its whole header — order it heading, facts, verdict | `no_kv_block_renders_at_column_zero_under_a_heading` (`crates/cfgd/src/cli/tests.rs`) |
| A **recorded scope's tokens paint like every other owner** | the kv slot takes `KvPair::scope_valued`, a table column `Table::owner_column`; a value that is not a `kind:name` list renders plain rather than half-styled | `every_recorded_scope_slot_declares_its_owner_tokens` (`crates/cfgd/src/cli/tests.rs`) |
...
| An **annotation states a fact the row cannot show** | `KvPair::annotated`'s muted `(note)` earns its place only by saying something the key, the value and the neighbouring rows do not already say: `Profile  base (from source:team-config)` names an owner nothing else states, `Locked Commit  9f3c1ab2 (same as last commit)` restates a comparison two visible rows already answer. A slot's verdict is recorded in the pin's table, so a new one is classified before it ships | `every_annotated_kv_slot_states_a_fact_its_row_cannot_show` (`crates/cfgd/src/cli/tests.rs`) |
...
| A table **data row ends on its last glyph** | only the HEADER keeps the pad its widest cell earned, because the `──` rule spans that width; a data row has nothing to its right, so a trailing pad is bytes no reader sees. A physical line with no glyph in any column is emitted as nothing. The shipped population the second pin walks is every derived golden root AND `docs/**/*.md`, so a doc block pasted from a regressed renderer is caught with the goldens | `every_table_shape_rules_the_full_width_of_its_grid` (`output/renderer/table.rs`), `every_trailing_space_in_a_golden_belongs_to_a_table_header` (`output/tests/fences.rs`) |
| A **module's Scripts inventory renders through the one composer** | `cfgd_core::modules::scripts_section` builds the `Scripts` section wherever a module's declared hooks are listed, in `Condensed` or `Full` form, and `modules::post_apply_scripts_section` is its `SectionGuard` twin for the module-approval screen; `modules/surfaces.rs` is the only file that reads the inventory or names that section, and outside `output/` a site painting a script body block says why with `// script-body-ok:`. A confirmation line for ONE script an invocation handed a setter is no inventory and stays outside the rule | `every_scripts_inventory_a_surface_renders_comes_from_the_one_composer` (`crates/cfgd/src/cli/tests.rs`) |
