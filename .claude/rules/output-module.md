...
- `Doc::script_steps(steps, form)` / `SectionBuilder::subsection_annotated(name, note, build)` — the buffered slots a module's declared scripts render through: the renderer owns each step's muted marker line, its bash highlighting and the blank line between steps, and the annotated heading carries the hook's step count (`postApply (7)`). Reach both through `cfgd_core::modules::scripts_section`.
...
| A **module's Scripts inventory renders through the one composer** | `cfgd_core::modules::scripts_section` builds the `Scripts` section wherever a module's declared hooks are listed, in `Condensed` or `Full` form; `modules/surfaces.rs` is the only file that reads the inventory or names that section, and outside `output/` a site painting a script body block says why with `// script-body-ok:`. A confirmation line for ONE script an invocation handed a setter is no inventory and stays outside the rule | `every_scripts_inventory_a_surface_renders_comes_from_the_one_composer` (`crates/cfgd/src/cli/tests.rs`) |
