...
- Methods:
  ...
  - `Doc::paragraph(text)` — a prose paragraph: wrapped body text with no glyph, no key column and no verbatim contract, for what a documentation surface says ABOUT the thing the heading above it just named (`cfgd explain`). Painted `theme.muted` per physical line, so no SGR run stays open across a wrap. Binds to a heading directly above it the way a kv block does, and an empty string emits nothing. Buffered-`Doc` only — the one producer builds a `Doc`, so there is no streaming `Printer::paragraph`
