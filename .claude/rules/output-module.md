...
- `status_owner_with(role, OwnerLabel, |f| …)` (`Doc` / `SectionBuilder`) — the ONE status-row slot an owner token may occupy as the SUBJECT: the renderer paints its three slots instead of the row's single role coat, and the plain `kind:name` still travels as the row's `subject`. A token inside a sentence keeps the plain slot. `every_status_row_naming_an_owner_takes_the_owner_subject_slot` pins it.
