...
- `section(name)` — a `SectionGuard` (drop ends it) carrying `bullet`/`kv`/`kv_block`/`command_list`/`hint`/`note`/`table`/`code_block` plus `.diff` and `.syntax_highlight`. A nested plain section paints its heading `theme.secondary`, so nesting reads from styling alone.
...
| **One alignment column per report**, not per phase — and every detail-bearing ROW SHAPE pads to it | measured once over every action the run will print, claimed through `Printer::report_column`, pseudo-phases included; a bullet's trailing detail pads to the same claim through `Emitting::bullet_column`. The claim, the trailing allowance and the subject budget are one computation (see `shared-utils.md`, `report_align_width`); a too-wide subject WRAPS, never cuts, and neither widens nor withdraws the column | `every_detail_bearing_row_of_a_report_lands_in_the_reports_one_column` (`reconciler/run/tests.rs`) |
