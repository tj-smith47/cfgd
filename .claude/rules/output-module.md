...
the "Caveats" heading painted `theme.warning` + bold — the one heading slot no other
section uses). `cli::plan_ops::print_caveats` is the one assembler for a real `cfgd apply`
