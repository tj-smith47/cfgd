...
- `SectionGuard::kv_rows(rows)` / `Doc::kv_rows(rows)` / `SectionBuilder::kv_rows(rows)` — `kv_block` over hand-built `KvPair`s, and how a row reaches the four renderer-owned slots: `annotated(key, value, note)` (a muted parenthesised note about the value), `role_valued(key, value, role)` (the VALUE tinted by what it says, through the same `role_glyph` mapping every role-styled surface reads), `nested(key, value)` (indented to say it belongs to the row above) and `owner_valued(key, owners)` (the VALUE as one or more `kind:name` tokens in `OwnerLabel`'s tri-colour coat, joined by `Owner::TOKEN_SEPARATOR` — the ONE way an owner token occupies a kv value, so `status <module>`'s `Scope` row spells an owner exactly as the apply tree's group heading and the Managed Resources Owner column do). **A caller never paints or indents one itself** — the fold would eat the coat, and a hand-padded key misaligns the block. All four are `#[serde(skip)]`, display-only (the plain `value` is what `-o json` and every colourless path read), and why `Printer::muted` no longer exists.
...
| **One alignment column per report**, not per phase — and every detail-bearing ROW SHAPE pads to it | the elapsed/detail column is measured once over every action the run will print and claimed through `Printer::report_column`, so it never moves x position between phases — including the pseudo-phases (`Backups`, `cfgd:env`) a `Plan` does not carry. A produced count renders as a BULLET's trailing detail (`- write ~/.cfgd.env — 3 vars, 3 aliases`), and `- ` is exactly as wide as a glyph and its space, so it pads to the same claim through `Emitting::bullet_column`. The claim is priced against what a row may print BESIDE the column, and only over rows that can print it: a wait reason over the pairs `lanes::phase_wait_reasons` can word, a produced count over `widest_produced_detail`, joined per phase. The subject budget, the allowance and the claim are one computation (`Printer::subject_budget_floor` = line minus glyph minus `WAIT_FRAMING_WIDTH`, halved; `reconciler::report_subject_budget` widens it to what the report's own allowance leaves and the claim holds it). A subject naming more operands than the line holds is not cut — it WRAPS — so the claim is priced over each subject's first physical row, which is the whole subject exactly when it fits the budget: a wrapped row carries its detail and its duration on its own last physical row and can neither widen the column nor withdraw it, and `group_column` narrows rather than drops | `report_align_width_spans_every_phase`, `report_align_width_ignores_a_filtered_out_phase`, `every_detail_bearing_row_of_a_report_lands_in_the_reports_one_column`, `a_report_wide_enough_for_its_wait_reasons_keeps_its_one_column` (`reconciler/run/tests.rs`), `a_wait_reason_is_priced_only_over_rows_it_can_name` (`reconciler/lanes.rs`) |
...
**A command that closes with the shared rollup opens with the shared header.** `cfgd backup restore` took `render_run_rollup`'s footer — the `✓ Restore complete — 1 action succeeded` verdict — while printing none of the `Config` / `Sources` / `Profile` / `Modules` rows `ApplyRun::header` exists for, so the one verb that overwrites live data was also the one that never said which config it acted under.

Both ends come from `reconciler::ApplyRun`. A run whose body the caller renders and whose work is neither a `Plan` action nor a `spec.backups[]` unit builds one through `ApplyRun::unplanned(ctx, actions)` — never a synthesized empty `Plan`, and never a second header renderer — `output::config_header_rows` is the ONE builder of those four rows, on every surface. `RunContext::subject` is what puts the acted-on unit in the title (`Restore: notes`), and `RunContext::unit_source` puts that unit's declared path in the header as a `Source` row after the four shared ones — an INPUT fact of the run, so it heads the run rather than trailing an action row as a `Destination` kv; the header renders BEFORE any confirmation prompt, because an operator agreeing to overwrite live data is agreeing under a config and a profile.
...
| A **kv value that is a link never hand-builds the fallback** | `KvPair::linked(key, text, url)` owns the OSC 8 slot: the short `text` where the theme carries hyperlinks, the `url` ITSELF everywhere else — never `KvPair::new("Docs", path)`, which prints a repo-relative pointer no terminal auto-links and no reader can paste into a browser. The capability is DETECTED (`output::terminal_supports_hyperlinks`, stamped onto the theme by the two production `Printer` constructors alone) and gated by colour, so every capture and every `--color never` run reads the plain URL | `every_docs_pointer_the_cli_renders_goes_through_the_linked_slot` (`crates/cfgd/src/cli/tests.rs`); `a_linked_value_under_a_hyperlink_theme_wraps_its_text_in_osc_8` and `a_linked_value_without_hyperlinks_prints_the_url_itself` (`output/tests/hyperlinks.rs`) pin both halves |
