...
- Methods:
  ...
  - `printer.status_simple(role, subject)` — concise status line; `role: Role::{Ok, Info, Warn, Fail, Skipped, Pending, Running, Accent, Secondary}`. `Accent` = "attention without alarm" (orange-family); `Secondary` = "structural pivot / label / identifier" (pink/magenta-family). Both have no icon and are suppressed at `Verbosity::Quiet` like every non-`Fail` role. The SUBJECT is stripped of foreign ANSI at the renderer boundary, the same fold `status`'s builder path, the buffered `Doc` path and the live-row path all apply — so a gateway JSON field or a tool's captured stderr echoed into a subject cannot erase the line it is written on, and cannot measure zero columns against the alignment the section pads to. A kv VALUE is NOT stripped and cannot be: that slot deliberately carries caller-composed styling (`printer.muted`), so a call site echoing an untrusted value through `kv` folds it itself.
