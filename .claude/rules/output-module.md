...
| A **declared script body renders through the one composer** | `cfgd_core::modules::scripts_section` builds the `Scripts` section for every human surface listing a module's hooks, in `Condensed` or `Full` form; outside `output/` nothing paints a script body for itself, and a site that does says why with `// script-body-ok:` | `every_declared_script_body_a_surface_renders_comes_from_the_one_composer` (`crates/cfgd/src/cli/tests.rs`) |
