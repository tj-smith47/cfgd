...
- `diff(old, new)` / `syntax_highlight(code, lang)` — nest at whatever depth is ambient, flushing any pending section header first. A highlighted block is painted in the printer's own preset through `Theme::syntect_theme` (the mapping is the table on `Theme`; the bundled `.tmTheme` assets and their licences are in `output/tmthemes/THIRD_PARTY.md`), and each line closes on a reset; `minimal` and an asset that will not parse render plain. Never name a syntect theme at a call site. `every_preset_resolves_the_syntect_theme_it_names` walks the presets.
