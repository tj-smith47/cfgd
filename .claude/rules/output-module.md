...
| A **heading owns every block under it until a row is printed** | a heading's prose and the facts after it both indent beneath it; a ROW ends the scope, so a verb's closing facts block stays at the report's own depth. A surface ordering heading, verdict, facts loses the binding for its whole header — order it heading, facts, verdict | `no_kv_block_renders_at_column_zero_under_a_heading` (`crates/cfgd/src/cli/tests.rs`) |
