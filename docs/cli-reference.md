...
`--show-values` is the per-verb alias for the global `--mask-env-values none`
(and conflicts with it). It renders the same inventories with each declared alias and env
...
A row the store already holds but this run's own scope could not re-check (a bare legacy module id outside every configurator the scan evaluated, or — in a `--module` run — a resource this run's scope owns that its narrower check never re-examined) is never silently dropped: it renders beside the live findings under its stored `expected`/`actual`, and prices into `5` exactly like a fresh finding. `diff`'s, `verify`'s and `status --scan`'s (both full and `--module`) `-o json` payloads all carry these under a `standing` key alongside the live findings; `status`'s `drift` array (or `--module`'s `drift`/`driftChecked` section) also folds them in for rendering, so a consumer reading `drift` alone still sees them and one reading `standing` sees which of `drift`'s rows were carried forward rather than freshly found.
