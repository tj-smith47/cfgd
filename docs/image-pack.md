...
or package managers. CI produces the directory (`go build` for a binary, `pip install --target`
for site-packages, `npm ci` for node_modules); `cfgd image pack` turns that directory into a
...
| `--label k=v` | Image config label (repeatable; lands in `config.Labels`) |
| `--annotation k=v` | Manifest annotation (repeatable; lands in `manifest.annotations`) |
...
## Worked example: Go binary service
...
- name: Pack binary image
  run: |
    cfgd image pack ./out \
      registry.example.com/myapp/server:${{ github.sha }} \
...
- name: Pack assets image
  run: |
    cfgd image pack ./out-assets \
      registry.example.com/myapp/assets:${{ github.sha }} \
      --sign --attest \
...
kind: Pod
spec:
  # root-in-pod is not root-on-node (KEP-127)
  hostUsers: false
  volumes:
    - name: app
      image:
        reference: registry.example.com/myapp/server@sha256:3a7b9c4d...
        pullPolicy: IfNotPresent
    - name: assets
      image:
        reference: registry.example.com/myapp/assets@sha256:7f2e1a9b...
        pullPolicy: IfNotPresent
    - name: tmp
...
| `RUN adduser -D -u 1000 myapp` | `nonroot` tag bakes this in |
| `COPY /myapp /app/myapp` | `volume.image` mount of `myapp/server@sha256:...` |
| `COPY /web /app/web` | `volume.image` mount of `myapp/assets@sha256:...` |
| `ENTRYPOINT ["/app/myapp"]` | `command:` field |
...
In Kubernetes 1.33 the gate is beta but off by default.
...
```bash
$ cfgd image pack ./out \
    registry.example.com/myapp/server:abc123 \
    --entrypoint /app/myapp --lock
Pack Image
  Directory  ./out
...
images:
  - reference: registry.example.com/myapp/server:abc123
    digest: sha256:3a7b9c4d...
    pinned: registry.example.com/myapp/server@sha256:3a7b9c4d...
