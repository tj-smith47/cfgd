...
Every tool carries annotations a client can gate on: `plan` is `readOnlyHint`, `apply` is `destructiveHint`, and the `* remove` / `rollback` commands are marked non-idempotent. `backup run` joins them — retention pruning removes superseded snapshots, its hooks stop and start services, and repeating it takes another snapshot rather than being a no-op. A client can prompt on a write without prompting on a preview.
