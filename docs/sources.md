...
- `Notify`: record a pending decision, send notification, don't apply. Whichever run classifies the item first records it — the daemon's tick, or a `cfgd apply` that reaches the new item before the daemon does — so the item is withheld from the very first plan that sees it and is answerable with `cfgd decide` straight away. `cfgd plan` writes nothing, so it lists the item as pending without recording it and leaves the row to the apply that follows. The desktop notification is the daemon's; an apply shows you the same item on screen instead.
