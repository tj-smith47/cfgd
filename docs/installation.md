...
curl -L -o cfgd.tar.gz \
  https://github.com/tj-smith47/cfgd/releases/latest/download/cfgd-0.6.2-linux-amd64.tar.gz
tar -xzf cfgd.tar.gz
...
```powershell
Invoke-WebRequest -Uri https://github.com/tj-smith47/cfgd/releases/latest/download/cfgd-0.6.2-windows-amd64.zip -OutFile cfgd.zip
...
example `cfgd-0.6.2-linux-amd64.tar.gz`) the release publishes:
...
```sh
VER=0.6.2; ARCH=amd64; OS=linux          # adjust: amd64|arm64, linux|darwin|windows
...
- The issuer is the GitHub Actions OIDC provider, and the
  `--certificate-identity-regexp` pins the signer to cfgd's three signing
  workflows, each on its legitimate ref: `publish-crate.yml` on `master` (the
  per-crate leg that signs stable assets), `release.yml` on a `v*` tag (the
...
> **v0.6.2 with the cosign CLI installed:** the v0.6.2 binary pinned the signer
...
> fails closed on v0.6.2 when `cosign` is present (no SHA256 fallback — that is
> the intended fail-closed behavior). Reinstall once via any install method above
> to get the v0.6.2+ binary, which pins the identity at the repository level and
