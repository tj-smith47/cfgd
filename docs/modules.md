...
| `minVersion` | no | string | Minimum acceptable version (semver). Also checked LIVE: `cfgd diff` / `status --scan` / `verify` report an installed copy below the floor as drift (`want: 0.9, have: 0.8.3`), and a manager that reports no installed version at all as a check that could not run |
