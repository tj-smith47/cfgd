...
fn cmd_generate_scan_only_with_empty_home() {
    ...
    let (printer, buf) = PrinterV2::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
    let args = GenerateArgs {
        target: None,
        model: None,
        provider: None,
...
fn handle_present_yaml_auto_accept() {
    let (printer, _buf) = PrinterV2::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
...
mod cmd_generate_mockito {
    ...
    fn cmd_generate_aborts_when_consent_prompt_declined() {
        ...
        // PrinterV2::for_test_at(cfgd_core::output_v2::Verbosity::Normal) prompt_confirm returns Err in non-interactive
