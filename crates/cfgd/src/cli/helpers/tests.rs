...
fn copy_files_to_dir_missing_source_errors() {
    let dst_dir = tempdir().unwrap();
    let err =
        copy_files_to_dir(&["/tmp/cfgd-nonexistent-9999.txt".into()], dst_dir.path()).unwrap_err();
    assert!(
...
fn resolve_desired_state_apply_and_read_compute_same_module_set() {
    ...
    let mut read_modules: Vec<String> = read_side.modules.iter().map(|m| m.name.clone()).collect();
    apply_modules.sort();
    read_modules.sort();
    assert_eq!(
