...
fn cmd_module_list_empty() {
    ...
    cmd_module_list(&cli, &v2_printer).unwrap();
    drop(v2_printer);
...
fn cmd_module_show_not_found() {
    ...
    let err = cmd_module_show(&cli, &v2_printer, "ghost", false).unwrap_err();
    assert!(
        err.to_string().contains("not found"),
...
fn cmd_module_show_displays_details() {
    ...
    cmd_module_show(&cli, &v2_printer, "devtools", false).unwrap();
    drop(v2_printer);
...
fn cmd_module_show_with_available_hint() {
    ...
    let err = cmd_module_show(&cli, &v2_printer, "missing", false).unwrap_err();
    drop(v2_printer);
    let output = v2_buf.lock().unwrap();
    assert!(
...
fn cmd_module_show_env_masking() {
    ...
    cmd_module_show(&cli, &v2_printer, "secrets-mod", false).unwrap();
    drop(v2_printer);
    let output = v2_buf.lock().unwrap();
    assert!(
        output.contains("***"),
...
fn cmd_module_show_env_unmasked() {
    ...
    cmd_module_show(&cli, &v2_printer, "env-mod", true).unwrap();
    drop(v2_printer);
    let output = v2_buf.lock().unwrap();
    assert!(
...
fn cmd_module_show_json_schema() {
    ...
    cmd_module_show(&cli, &v2_printer, "jmod", false).unwrap();
    drop(v2_printer);
...
fn cmd_module_update_add_env() {
    ...
    cmd_module_update_local(&cli, &v2_printer, &args).unwrap();
...
fn cmd_module_update_nonexistent_fails() {
    ...
    let err = cmd_module_update_local(&cli, &v2_printer, &args).unwrap_err();
    assert!(
        err.to_string().contains("not found"),
        "should report not found, got: {err}"
    );
...
fn cmd_module_create_with_env_and_aliases() {
    ...
    cmd_module_create(&cli, &v2_printer, &args).unwrap();
    drop(v2_printer);
...
fn cmd_module_create_invalid_name_fails() {
    ...
    let err = cmd_module_create(&cli, &v2_printer, &args).unwrap_err();
    assert!(
...
fn cmd_module_delete_nonexistent_fails() {
    ...
    let err = cmd_module_delete(&cli, &v2_printer, "ghost", true, false).unwrap_err();
    assert!(
        err.to_string().contains("not found"),
        "should report not found, got: {err}"
    );
...
fn cmd_module_delete_invalid_name_fails() {
    ...
    let err = cmd_module_delete(&cli, &v2_printer, "-bad", true, false).unwrap_err();
    assert!(
        err.to_string().contains("cannot start with"),
        "should reject invalid name, got: {err}"
    );
...
fn cmd_module_registry_add_duplicate_is_noop() {
    let dir = setup_config_dir();
    let cli = test_cli(dir.path());
    let v2_printer1 = make_v2_printer();
...
fn cmd_module_registry_add_no_config_fails() {
    ...
    let err = cmd_module_registry_add(
        &cli,
        &v2_printer,
...
fn cmd_module_registry_remove_existing() {
    ...
    let (v2_printer2, buf2) =
        cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
    cmd_module_registry_remove(&cli, &v2_printer2, "myrepo").unwrap();
    drop(v2_printer2);
...
fn cmd_module_registry_remove_not_found() {
    ...
    cmd_module_registry_remove(&cli, &v2_printer, "nonexistent").unwrap();
    drop(v2_printer);
...
fn cmd_module_registry_rename_success() {
    ...
    let (v2_printer2, buf2) =
        cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
    cmd_module_registry_rename(&cli, &v2_printer2, "old-name", "new-name").unwrap();
    drop(v2_printer2);
...
fn cmd_module_registry_rename_not_found_fails() {
    ...
    let err = cmd_module_registry_rename(&cli, &v2_printer, "ghost", "new").unwrap_err();
    assert!(
        err.to_string().contains("not found"),
        "should report not found, got: {err}"
    );
...
fn cmd_module_registry_rename_target_exists_fails() {
    ...
    let err = cmd_module_registry_rename(&cli, &v2_printer2, "alpha", "beta").unwrap_err();
    assert!(
...
fn cmd_module_registry_list_empty() {
    ...
    cmd_module_registry_list(&cli, &v2_printer).unwrap();
    drop(v2_printer);
...
fn cmd_module_registry_list_with_entries() {
    ...
    let (v2_printer2, buf2) =
        cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
    cmd_module_registry_list(&cli, &v2_printer2).unwrap();
    drop(v2_printer2);
...
fn cmd_module_registry_list_json() {
    ...
    let (v2_printer2, cap) = cfgd_core::output_v2::Printer::for_test_doc();
    cmd_module_registry_list(&cli_json, &v2_printer2).unwrap();
    drop(v2_printer2);
...
fn cmd_module_search_no_config_fails() {
    ...
    let err = cmd_module_search(&cli, &v2_printer, "test").unwrap_err();
    assert!(
...
fn cmd_module_search_no_registries() {
    ...
    cmd_module_search(&cli, &v2_printer, "test").unwrap();
    drop(v2_printer);
...
fn cmd_module_registry_rename_cascades_to_profiles() {
    ...
    cmd_module_registry_add(&cli, &v2_printer, "https://example.com/r.git", Some("old")).unwrap();
    ...
    cmd_module_registry_rename(&cli, &v2_printer, "old", "fresh").unwrap();
...
fn cmd_module_show_json_with_lockfile_entry() {
    ...
    cmd_module_show(&cli, &v2_printer, "remote-mod", false).unwrap();
    drop(v2_printer);
...
fn cmd_module_show_table_with_lockfile_entry() {
    ...
    cmd_module_show(&cli, &v2_printer, "locked-mod", false).unwrap();
    drop(v2_printer);
...
fn cmd_module_show_aliases() {
    ...
    cmd_module_show(&cli, &v2_printer, "alias-mod", false).unwrap();
    drop(v2_printer);
...
fn cmd_module_show_scripts() {
    ...
    cmd_module_show(&cli, &v2_printer, "script-mod", false).unwrap();
    drop(v2_printer);
...
fn cmd_module_show_files_with_git_source() {
    ...
    cmd_module_show(&cli, &v2_printer, "git-file-mod", false).unwrap();
    drop(v2_printer);
...
fn cmd_module_create_duplicate_name_fails() {
    ...
    let v2_printer2 = make_v2_printer();
    let err = cmd_module_create(&cli, &v2_printer2, &args).unwrap_err();
    assert!(
        err.to_string().contains("already exists"),
        "should report already exists, got: {err}"
    );
...
fn cmd_module_delete_with_yes_succeeds() {
    ...
    cmd_module_delete(&cli, &v2_printer, "to-delete", true, false).unwrap();
    drop(v2_printer);
...
fn cmd_module_delete_without_yes_and_prompt_confirmed_proceeds_with_deletion() {
    ...
    cmd_module_delete(&cli, &v2_printer, "prompt-yes-mod", false, false).unwrap();
    drop(v2_printer);
...
fn cmd_module_edit_with_invalid_yaml_and_prompt_declined_breaks_with_warning() {
    ...
    cmd_module_edit(&cli, &v2_printer, "edit-broken")
        .expect("edit must return Ok even on Save-with-errors");
    drop(v2_printer);

    let output = v2_buf.human();
    assert!(
...
fn cmd_module_create_with_apply_and_yes_drives_full_apply_sequence() {
    ...
    cmd_module_create(&cli, &v2_printer, &args)
...
fn cmd_module_create_interactive_drives_full_prompt_sequence_via_harness() {
    ...
    let (v2_printer, buf) = cfgd_core::output_v2::Printer::for_test_with_prompt_responses_at(
        vec![
            cfgd_core::output_v2::PromptAnswer::Text("Interactive build module".to_string()),
            cfgd_core::output_v2::PromptAnswer::Text("nodejs,git".to_string()),
            cfgd_core::output_v2::PromptAnswer::Text("ripgrep".to_string()),
    ...
    cmd_module_create(&cli, &v2_printer, &args).expect("interactive create should succeed");
    drop(v2_printer);
...
fn cmd_module_delete_without_yes_and_prompt_declined_returns_cancelled() {
    ...
    cmd_module_delete(&cli, &v2_printer, "prompt-no-mod", false, false).unwrap();
    drop(v2_printer);
...
fn cmd_module_delete_refused_when_profile_references() {
    ...
    let err = cmd_module_delete(&cli, &v2_printer, "referenced", true, false).unwrap_err();
    assert!(
...
fn cmd_module_delete_with_purge() {
    ...
    cmd_module_delete(&cli, &v2_printer, "purge-mod", true, true).unwrap();
...
fn cmd_module_delete_restores_symlinked_files() {
    ...
    cmd_module_delete(&cli, &v2_printer, "restore-mod", true, false).unwrap();
...
fn cmd_module_delete_with_purge_removes_directory_target() {
    ...
    let (v2_printer, buf) =
        cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
    cmd_module_delete(&cli, &v2_printer, "purge-dir-mod", true, true).unwrap();
...
fn cmd_module_delete_default_mode_restores_directory_source_via_copy_dir() {
    ...
    let (v2_printer, buf) =
        cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
    cmd_module_delete(&cli, &v2_printer, "restore-dir-mod", true, false).unwrap();
...
fn cmd_module_delete_cleans_lockfile() {
    ...
    cmd_module_delete(&cli, &v2_printer, "lock-mod", true, false).unwrap();
...
fn cmd_module_show_renders_platform_filtered_and_resolved_packages() {
    ...
    cmd_module_show(&cli, &v2_printer, "rich", false).unwrap();
    drop(v2_printer);
...
fn cmd_module_registry_remove_no_config_fails() {
    ...
    let err = cmd_module_registry_remove(&cli, &v2_printer, "test").unwrap_err();
    assert!(
        err.to_string().contains("cfgd.yaml"),
        "should fail without config, got: {err}"
    );
...
fn cmd_module_registry_remove_warns_profile_refs() {
    ...
    let (v2_printer2, buf2) =
        cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
    cmd_module_registry_remove(&cli, &v2_printer2, "team").unwrap();
    drop(v2_printer2);
...
fn cmd_module_show_json_depends() {
    ...
    cmd_module_show(&cli, &v2_printer, "dep-show", false).unwrap();
    drop(v2_printer);
...
fn cmd_module_registry_rename_no_config_fails() {
    ...
    let err = cmd_module_registry_rename(&cli, &v2_printer, "old", "new").unwrap_err();
    assert!(
        err.to_string().contains("cfgd.yaml"),
        "should fail without config, got: {err}"
    );
...
fn print_module_review_summary_shows_dependency_list_when_present() {
    ...
    super::registry::print_module_review_summary(&v2_printer, "m", &module, "c", "i");
    drop(v2_printer);
    let out = buf.lock().unwrap().clone();
...
mod cmd_module_add_remote_local_bare {
    ...
    fn cmd_module_add_remote_against_local_bare_adds_to_lockfile_and_profile() {
        ...
        let v2_printer = make_v2_printer();
        cmd_module_add_remote(&cli, &v2_printer, &url, None, true, true)
    ...
    fn cmd_module_add_remote_is_idempotent_when_module_already_in_lockfile() {
        ...
        let cli = test_cli(work.path());
        let v2_printer1 = make_v2_printer();
        cmd_module_add_remote(&cli, &v2_printer1, &url, None, true, true).unwrap();
        // Second invocation hits the "already in lockfile" early return.
        let (v2_printer2, buf) =
            cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
        cmd_module_add_remote(&cli, &v2_printer2, &url, None, true, true)
    ...
    fn cmd_module_add_remote_bails_when_local_module_with_same_name_exists() {
        ...
        let cli = test_cli(work.path());
        let v2_printer = make_v2_printer();
        let err = cmd_module_add_remote(&cli, &v2_printer, &url, None, true, true)
    ...
    fn cmd_module_upgrade_against_local_bare_replaces_lockfile_entry() {
        ...
        let cli = test_cli(work.path());
        let v2_printer = make_v2_printer();
        cmd_module_add_remote(&cli, &v2_printer, &url_v1, None, true, true).unwrap();
        ...
        // Upgrade to v2.0.0.
        cmd_module_upgrade(&cli, &v2_printer, "mymod", Some("v2.0.0"), true, true)
    ...
    fn cmd_module_upgrade_returns_early_when_module_not_in_lockfile() {
        ...
        let cli = test_cli(work.path());
        let v2_printer = make_v2_printer();
        let err = cmd_module_upgrade(&cli, &v2_printer, "ghost", Some("v9.9.9"), true, true)
    ...
    fn cmd_module_upgrade_with_no_new_ref_logs_latest_head_and_short_circuits() {
        ...
        let cli = test_cli(work.path());
        let v2_printer1 = make_v2_printer();
        cmd_module_add_remote(&cli, &v2_printer1, &url_v1, None, true, true).unwrap();

        let lock_v1 = std::fs::read_to_string(work.path().join("modules.lock")).unwrap();

        let (v2_printer2, buf2) =
            cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
        cmd_module_upgrade(&cli, &v2_printer2, "mymod", None, true, true)
    ...
    fn cmd_module_upgrade_returns_early_when_already_at_target_commit() {
        ...
        let (v2_printer2, buf2) =
            cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
        cmd_module_upgrade(&cli, &v2_printer2, "mymod", Some("v1.0.0"), true, true)
    ...
    fn cmd_module_upgrade_bails_when_target_is_a_local_module() {
        ...
        let cli = test_cli(work.path());
        let v2_printer = make_v2_printer();
        let err = cmd_module_upgrade(&cli, &v2_printer, "localmod", Some("v1"), true, true)
...
mod cmd_module_add_from_registry_local {
    ...
    fn cmd_module_add_from_registry_explicit_tag_writes_lockfile_and_profile() {
        ...
        let (v2_printer, buf) =
            cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
        cmd_module_add_from_registry(&cli, &v2_printer, "myreg/alpha@v1.0.0", true, true)
    ...
    fn cmd_module_add_from_registry_no_tag_resolves_latest_version() {
        ...
        cmd_module_add_from_registry(&cli, &v2_printer, "myreg/beta", true, true)
    ...
    fn cmd_module_add_from_registry_unknown_registry_errors() {
        ...
        let v2_printer = make_v2_printer();
        let err =
            cmd_module_add_from_registry(&cli, &v2_printer, "ghost-reg/foo@v1.0.0", true, true)
    ...
    fn cmd_module_add_from_registry_invalid_reference_format_errors() {
        ...
        let err = cmd_module_add_from_registry(&cli, &v2_printer, "noslash", true, true)
    ...
    fn cmd_module_add_from_registry_unknown_module_errors_when_no_tags() {
        ...
        let err = cmd_module_add_from_registry(&cli, &v2_printer, "myreg/beta", true, true)
    ...
    fn cmd_module_search_returns_matching_module_in_table() {
        ...
        let (v2_printer, buf) =
            cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
        cmd_module_search(&cli, &v2_printer, "alph").expect("search should succeed");
        drop(v2_printer);
    ...
    fn cmd_module_search_reports_no_matches_when_query_misses() {
        ...
        let (v2_printer, buf) =
            cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
        cmd_module_search(&cli, &v2_printer, "no-such-name").expect("search should succeed");
        drop(v2_printer);
    ...
    fn cmd_module_search_wide_format_includes_registry_column() {
        ...
        cmd_module_search(&cli, &v2_printer, "alph").expect("wide search should succeed");
        drop(v2_printer);
    ...
    fn cmd_module_search_json_emits_results_array() {
        ...
        let (v2_printer, cap) = cfgd_core::output_v2::Printer::for_test_doc();
        cmd_module_search(&cli, &v2_printer, "alpha").expect("json search should succeed");
        drop(v2_printer);
    ...
    fn cmd_module_search_unreachable_registry_emits_failure_warning() {
        ...
        let (v2_printer, buf) =
            cfgd_core::output_v2::Printer::for_test_at(cfgd_core::output_v2::Verbosity::Normal);
        cmd_module_search(&cli, &v2_printer, "anything")
