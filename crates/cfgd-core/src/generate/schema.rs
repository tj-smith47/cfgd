...
mod tests {
    ...
    #[test]
    fn all_schemas_are_non_empty_valid_yaml() {
        for kind in [SchemaKind::Module, SchemaKind::Profile, SchemaKind::Config] {
            let schema = get_schema(kind);
            assert!(!schema.is_empty(), "{kind:?} schema is empty");
            let _: serde_yaml::Value = serde_yaml::from_str(schema)
                .unwrap_or_else(|e| panic!("{kind:?} schema is not valid YAML: {e}"));
            assert!(
                schema.contains("apiVersion"),
